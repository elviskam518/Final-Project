...
def threshold_for_target_rate(y_prob: np.ndarray, target_rate: float) -> float:
    """
    找阈值 t，使得 mean(y_prob >= t) ~= target_rate
    例如 target_rate=0.30 -> 取 70% 分位数作为阈值
    """
...
def evaluate_model_at_threshold(model, data, threshold: float, device="cpu"):
    """
    用指定阈值评估：Acc/F1/PredRate + fairness(DI/EO)
    """
    model.eval()
    model = model.to(device)
...
def run_experiment(csv_path="tech_diversity_hiring_data.csv"):
    ...
    # ==========================================================================
    # EXPERIMENT: Feature Removal Ablation
    # Purpose: Prove that our Fair CVAE is NOT just ignoring biased features,
    #          but learning to extract useful info while removing bias signal.
    # ==========================================================================
